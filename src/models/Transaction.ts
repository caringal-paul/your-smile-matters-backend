...
// For notification
transactionSchema.post("save", async function (doc) {
  if (doc.transaction_type === "Refund") return;
  ...
  const ref = doc.transaction_reference;
  const base = {
    booking_id: doc.booking_id,
    transaction_id: doc._id as Types.ObjectId,
    ...
    case "Pending":
      if ((doc as any).wasNew) {
        await createNotification({
          ...
          message: `${customerNo} submitted a payment of ₱${doc.amount.toLocaleString()} for appointment ${bookingRef}. Ref: ${ref}.`,
        });
      }
      break;
    case "Completed":
      await createNotification({
        ...
        message: `${customerNo}'s payment of ₱${doc.amount.toLocaleString()} is now verified. Booking: ${bookingRef} | Txn: ${ref}.`,
      });
      break;
    case "Failed":
      await createNotification({
        ...
        message: `${customerNo}'s payment of ₱${doc.amount.toLocaleString()} could not be processed. Booking: ${bookingRef} | Txn: ${ref}. Reason: ${doc.failure_reason ?? "N/A"}.`,
      });
      break;
  }
});
