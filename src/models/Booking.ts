...
bookingSchema.post("save", async function (doc) {
  ...
  const serviceIds = doc.services.map((s) => s.service_id);
  ...
  const ref = doc.booking_reference;
  const base = {
    booking_id: doc._id as Types.ObjectId,
  ...
  if ((doc as any).wasNew) {
    await createNotification({
      ...
      message: `${customerNo} placed a new appointment for ${servicesSummary}. Ref: ${ref}.`,
    });
    return;
  }
  ...
  switch (doc.status) {
    case "Confirmed":
      await createNotification({
        ...
        message: `${customerNo}'s appointment for ${servicesSummary} is now confirmed. Ref: ${ref}.`,
      });
      break;
    case "Cancelled":
      await createNotification({
        ...
        message: `${customerNo}'s appointment (${ref}) was cancelled. Reason: ${doc.cancelled_reason ?? "N/A"}.`,
      });
      break;
    case "Completed":
      await createNotification({
        ...
        message: `${customerNo}'s session for ${servicesSummary} is now completed. Ref: ${ref}.`,
      });
      break;
    case "Rescheduled":
      await createNotification({
        ...
        message: `${customerNo}'s appointment (${ref}) is pending a reschedule.`,
      });
      break;
  }
});
